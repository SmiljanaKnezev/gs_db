...
export REPO=renderedtext/grading_service
IMAGE=$(REPO):$(TAG)
...
HOME_DIR=/home/dev
WORKDIR=$(HOME_DIR)/grading_service
USER=dev
MIX_ENV=dev
...
image.test: image.build
	docker run --rm --name grading_service -p 4004:4000 -d $(IMAGE)
	# run some tests...
	docker kill grading_service

deploy: abort-on-uncommited-changes image.push config-gen
	kubectl apply -f deploy.yml
	kubectl get svc/grading-service
